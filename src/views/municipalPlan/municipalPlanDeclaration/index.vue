 ...
 * @Module:  src/views/municipalPlan/municipalPlanDeclaration/index
...
import { getAnnualTrainPlanSchoolInfoList, deleteAnnualTrainPlanInfo, annualTrainPlanSubmitCheck, getAnnualTrainPlanYearList } from '@/api/municipalPlan/municipalPlanDeclaration'
export default {
  components: {
    EtControlTable
  },
  ...
  beforeRouteEnter(to, from, next) {
    if (!(~from.path.indexOf('/municipalPlan/municipalPlanDeclaration'))) {
  ...
  beforeRouteLeave(to, from, next) {
    if (~to.path.indexOf('/municipalPlan/municipalPlanDeclaration')) {
  ...
  methods: {
    ...
    handleAddClick() {
      this.$router.push('/municipalPlan/municipalPlanDeclaration/add')
    },
    handleEditClick(row) {
      this.$router.push({
        path: '/municipalPlan/municipalPlanDeclaration/edit',
        query: { tId: row.id }
      })
    },
    handleDetailClick(row) {
      this.$router.push({
        path: '/municipalPlan/municipalPlanDeclaration/detail',
        query: { tId: row.id }
      })
    },
    handleRecheckClick(row) {
      this.$router.push({
        path: '/municipalPlan/municipalPlanDeclaration/recheck',
        query: { tId: row.id }
      })
    },
