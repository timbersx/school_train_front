<!--/*
 * @Name: TrainEventCheckInfo add
 * @Module:  src/views/trainEventCheckInfo/operations/edit
 * @Desc: TrainEventCheckInfo add
...
import { createTrainEventCheckInfo } from '@/api/traineventmanage/trainEventCheckInfo'
...
check_time:[
  ...
  methods: {
      init(){
Promise.all([getAuthList({limit: 100000000,offset: 0})])
.then(response=>{
  ...
  this.loading = false })
      ...
      handleSubmitForm() {
          this.$refs['dataForm'].validate(valid => {
              if (valid) {
                  this.loading = true
                  createTrainEventCheckInfo(this.formData).then(response => {
