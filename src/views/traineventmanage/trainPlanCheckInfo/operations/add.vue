<!--/*
 * @Name: TrainPlanCheckInfo add
 * @Module:  src/views/trainPlanCheckInfo/operations/edit
 * @Desc: TrainPlanCheckInfo add
...
import { createTrainPlanCheckInfo } from '@/api/traineventmanage/trainPlanCheckInfo'
...
current_check_status:[
  ...
  methods: {
      init(){
Promise.all([getAnnualTrainPlanInfoList({limit: 100000000,offset: 0}),getAuthList({limit: 100000000,offset: 0})])
.then(response=>{
  ...
  this.loading = false })
      ...
      handleSubmitForm() {
          this.$refs['dataForm'].validate(valid => {
              if (valid) {
                  this.loading = true
                  createTrainPlanCheckInfo(this.formData).then(response => {
