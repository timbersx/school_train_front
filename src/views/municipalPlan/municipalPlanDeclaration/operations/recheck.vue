  ...
  <div class="app-container">
    ...
    <el-tabs v-model="activeName">
        ...
        <div>
          ...
          >
            <el-row :gutter="15">
              ...
              <div v-if="isTSPX">
                <el-col v-for="(item, index) in propList" :key="index" :span="index>2 ? 22 : 11">
                  <el-form-item :label="$t('annualTrainPlanInfo.'+item)" :prop="item">
                    <el-input
                      v-model="form[item]"
                      :placeholder="index>2 ? '请输入内容，最多可以输入2048个字' : $t('table.input')"
                      :type="index>2 ? 'textarea' : ''"
                      :rows="5"
                    />
                  </el-form-item>
                </el-col>
      ...
      <el-tab-pane label="审核记录" name="second">
        <CheckTable :plan-id="$route.query.tId" />
      </el-tab-pane>
    </el-tabs>
...
import CheckTable from '@/components/EtPlanDetailMunicipal/components/CheckTable'
import { getAnnualTrainPlanTypeInfoList, updateAnnualTrainPlanInfo, readAnnualTrainPlanInfo, annualTrainPlanSubmitCheck, getAnnualTrainPlanYearList } from '@/api/traineventmanage/annualTrainPlanInfo'
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      planTypeList: [],
      propList: ['funds', 'organization', 'cooperation_organization',
  ...
  methods: {
    ...
    init() {
      ...
      Promise.all(promisetList).then(response => {
        ...
        const res2 = response[2].data
        res2.plan_type = res2.plan_type.id
        res2.year = res2.year.id
        delete res2.school
        this.fileList = res2.attachments.map(i => {
          i.name = i.local_file_name
    ...
    handleSave() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          let submitData = []
          if (formData.plan_type === 1) {
            submitData = [this.trainId, {
          ...
          if (formData.plan_type === 2) {
            const submitObj = {}
            this.propList.concat(['plan_type', 'name', 'year', 'director', 'start_date', 'end_date']).forEach(item => {
              submitObj[item] = formData[item]
            })
            submitData = [this.trainId, submitObj]
          }
          updateAnnualTrainPlanInfo(...submitData).then(response => {
            if (this.isCheck) {
