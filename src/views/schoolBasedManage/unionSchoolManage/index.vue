  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column
        ...
        :key="index"
        :prop="item"
        :label="$t(`unionSchoolManage.${item}`)"
      >
        <template slot-scope="scope">
...
import {
  getUnionschoolInfo,
  deleteUnionschool
} from '@/api/schoolBasedManage/unionSchoolManage'
...
export default {
  ...
  data() {
    return {
      ...
      listColumns: ['name', 'organizations'], // 表中显示的列
  ...
  methods: {
    ...
    getListData() {
      ...
      getUnionschoolInfo(this.apiQueryData).then(response => {
    ...
    handleDeleteClick(row) {
      if (JSON.stringify(this.deleteRoute) !== '{}') {
        this.$confirm(this.$t('confirm.delete'), this.$t('confirm.warning'), {
          ...
          .then(async() => {
            deleteUnionschool(row.id).then(response => {
