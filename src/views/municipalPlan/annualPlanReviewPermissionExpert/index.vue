...
<script>
import { getExpertGroupList, setExpertPermission, getExpertPermission,
//  getOrganizationTypeOrganizations,
  getIndicatorScoreTreeData } from '@/api/municipalPlan/annualPlanReviewPermissionExpert'
  ...
  methods: {
    init() {
      ...
      Promise.all([
        ...
        this.scoreTree = response[1].data
        this.loading = false
      }).catch(() => {
        this.loading = false
      })
    ...
    submitData() {
      ...
      this.loading = true
      setExpertPermission({ score_points, user: this.selectedUserID }).then(() => {
        this.loading = false
