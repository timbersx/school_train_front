...
export default {
  ...
  methods: {
    ...
    handleAddRecordsClick(row) {
      this.$router.push({ path: '/municipalEvent/municipalEventRecords/add', query: { pId: row.id }})
    },
    exportBtnClick(item) {
    ...
    handleCheckRecordsClick(row) {
      this.$router.push({ path: '/municipalEvent/municipalEventRecords/checkImplementationRecord', query: { pId: row.id }})
    },
