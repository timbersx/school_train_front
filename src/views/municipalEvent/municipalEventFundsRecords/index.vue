...
export default {
  ...
  methods: {
    ...
    handleAddRecordsClick(row) {
      this.$router.push({ path: '/municipalEvent/municipalEventFundsRecords/add', query: { pId: row.id }})
    },
    exportBtnClick(item) {
    ...
    handleCheckRecordsClick(row) {
      this.$router.push({ path: '/municipalEvent/municipalEventFundsRecords/checkFundsRecord', query: { pId: row.id }})
    },
