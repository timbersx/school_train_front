...
<template>
  <div class="app-container" v-loading="loading">
    ...
    <el-container class="form-container">
      <el-main>
        ...
        >
          ...
          <el-col :span="12">
            ...
            >
              <el-select
                ...
                filterable
                value-key="id"
                :filter-method="handelTrainPersonFilterChange"
              >
                <el-option
                ...
                <el-pagination
                  ...
                  @current-change="handleTrainPersonCurrentChange"
                />
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            ...
            <el-col :span="10">
                <el-select
                  ...
                  value-key="id"
                  multiple
                  :filter-method="speakerListFilter"
                >
                  <el-option
          ...
          <!-- <div v-if="choosedEnrollType == 'ZJZD'">
            <el-col :span="12">
              <el-form-item
          ...
          > -->
          <el-col :span="12" v-if="choosedEnrollType == 'XXTJ'">
            <el-form-item :label="$t('municipalEventCreation.setSchoolRecommendNumber')">
              <el-button
                type="primary"
                size="small"
                @click="handelRecommendClick"
                >选择</el-button
              >
            </el-form-item>
          </el-col>
          ...
          <div v-if="choosedEnrollType == 'ZZBM'">
            <el-col :span="12">
              <el-form-item
                :label="$t('trainEventInfo.enroll_amount')"
          ...
          <el-col :span="12">
            <el-form-item :label="$t('trainEventInfo.resources')" prop="resources">
              <el-button
                  type="primary"
                  size="small"
                  @click="handeleResourcePushClick"
                  >上传</el-button
                >
            </el-form-item>
          </el-col>
    ...
    />
    <school-recommend-dialog :visible.sync="schoolRecommendDialogVisible" :organizations="formData.organizations_details" @organizations="schoolRecommendDialogChange"/>

    <resource-pushing :visible.sync="resourcePushingDialogVisible" :resources="formData.resources" @attachments="getAttachmentListDialogChange" @attachments-repeat="getAttachmentListDialogRepeatChange"/>
  </div>
</template>
...
import { getAnnualTrainPlanSchoolInfoList } from "@/api/municipalPlan/municipalPlanDeclaration"
...
import SchoolRecommendDialog from './setSchoolRecommend';
import ResourcePushing from './resourcePushing';

export default {
  components: {
    Sticky,
    TeacherListDialog,
    SchoolRecommendDialog,
    ResourcePushing
  },
  data() {
    return {
      ...
      speakers_list: [],
      resourcesTemp: [],
      schoolRecommendorganizations: [],
      schoolRecommendDialogVisible: false,
      speakers_type: -1,
      limit: 10,
      rules: {
        name: [
  ...
  methods: {
    init() {
      Promise.all([
        ...
        this.speakers_type = response[9].data.speaker.id
    ...
    handleSubmitForm() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          this.loading = true;
          if(this.resourcesTemp){
            this.formData.resources = this.resourcesTemp
          }
          ...
          if(this.schoolRecommendorganizations.length > 0){
            let schoolRecommendorganizationsTemp = []
            this.schoolRecommendorganizations.map(item => schoolRecommendorganizationsTemp.push({organization: item.organization, amount: item.amount}))
            this.formData.organizations = schoolRecommendorganizationsTemp
          }else{
            let organizations = []
            this.formData.organizations.map(item => {
              if(typeof item === "object"){
                organizations.push({organization: item.organization})
              }else{
                organizations.push({organization: item})
              }
            })
            this.formData.organizations = organizations
          }
    ...
    handelRecommendClick() {
      this.schoolRecommendDialogVisible = true;
    },
    schoolRecommendDialogChange(val){
      this.schoolRecommendorganizations = val;
      this.schoolRecommendDialogVisible = false;
    },
    handelSpeakerChange(val){
      this.speakers_type = val
    ...
    handeleResourcePushClick(){
      this.resourcePushingDialogVisible = true
    },
    getAttachmentListDialogChange(val){
      this.resourcePushingDialogVisible = false,
      this.formData.resources = val
    },
    getAttachmentListDialogRepeatChange(val){
      this.resourcePushingDialogVisible = false,
      this.resourcesTemp = val
    },
    debounce(fn, delay) {
      let timer = null
      return function () {
    ...
    speakerListFilter(val){
      var search = val
      this.debounce(() => {
        getSpeakers({speaker: this.speakers_type}, {limit: this.limit, search: search}).then(res => {
          this.speakers_list = res.data
    ...
    handleCurrentChange(val) {
      getSpeakers({speaker: this.speakers_type}, {limit: this.limit, offset: this.limit * (val - 1)}).then(res => {
          this.speakers_list = res.data
          this.dataTotal = res.count
        })
    },
    handleTrainPersonCurrentChange(val){
      getSchoolTeachers({limit: this.limit, offset: this.limit * (val - 1)}).then(res => {
        this.school_teachers_Operations = res.data;
      })
    },
    handelTrainPersonFilterChange(val){
      var search = val
      this.debounce(() => {
        getSchoolTeachers({limit: this.limit, search: search}).then(res => {
          this.school_teachers_Operations = res.data;
        })
      }, 1000)()
    },
