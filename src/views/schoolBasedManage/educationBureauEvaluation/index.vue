...
<template>
  <div class="app-container">
      ...
      <el-tabs v-model="activeStatus" @tab-click="handleStatusClick">
        ...
        <el-tab-pane label="特色培训待审核" name="uniqueWaitCheck">
          ...
          >
            ...
            />
            <el-table-column
              prop="funds_expert.approve_comment"
              :label="$t(`educationBureauEvaluation.approve_comment`)"
            />
            <el-table-column
            ...
            >
              <template slot-scope="scope">
                <el-input v-model="scope.row.fast_education_funds" @blur="handelEducationFundsBlur(scope.row)" :placeholder="$t('table.input')"/>
              </template>
            </el-table-column>
    ...
    <batch-check :visible.sync='batchDialogVisible' :plans='listSelection' :activeStatus='activeStatus' @checked='handeCheckedChange'/>
...
export default {
  ...
  created() {
    let educationActiveStatus = sessionStorage.getItem('educationActiveStatus');
  ...
  methods: {
    ...
    handleFormatter(row, column) {
      ...
      switch (column.property) {
        ...
        case "status":
          let status = this.trainPlanCheckStatusList.find(item => item.code == row.status)
          return status.name;
