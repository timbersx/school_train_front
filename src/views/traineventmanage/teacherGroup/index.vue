...
<script>
import {
  getSchoolTeacherGroupInfoList,
  deleteSchoolTeacherGroupInfo
...
export default {
  ...
  filters: {
    getColumnContent(item, content) {
      switch (item) {
        case 'teachers':
          return content ? content.map(i => i.name).join('、') : ''
        default:
          return content
      }
    }
  ...
  data() {
    return {
      ...
      tableWidth: ['150px', ''],
      listColumns: ['name', 'teachers'], // 表中显示的列
  ...
  methods: {
    ...
    getListData() {
      ...
      getSchoolTeacherGroupInfoList(this.apiQueryData).then(response => {
    ...
    handleDeleteClick(row) {
      if (JSON.stringify(this.deleteRoute) !== '{}') {
        this.$confirm(this.$t('confirm.delete'), this.$t('confirm.warning'), {
          ...
          .then(async() => {
            deleteSchoolTeacherGroupInfo(row.id).then(response => {
