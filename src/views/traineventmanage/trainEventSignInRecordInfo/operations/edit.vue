<!--/*
 * @Name: TrainEventSignInRecordInfo edit
 * @Module:  src/views/trainEventSignInRecordInfo/operations/edit
 * @Desc: TrainEventSignInRecordInfo edit
...
import { readTrainEventSignInRecordInfo, updateTrainEventSignInRecordInfo } from '@/api/traineventmanage/trainEventSignInRecordInfo'
...
sign_in_time:[
  ...
  methods: {
      init(){
         Promise.all([getTrainEventInfoList({limit: 100000000,offset: 0}),getWjssoTeacherInfoList({limit: 100000000,offset: 0}),readTrainEventSignInRecordInfo(this.editId)])
         .then(response=>{
             this.train_event_Operations=response[0].data
this.teacher_Operations=response[1].data

             this.formData = response[2].data
             this.formData.train_event = this.formData.train_event==null?this.formData.train_event:this.formData.train_event.id
this.formData.teacher = this.formData.teacher==null?this.formData.teacher:this.formData.teacher.id
             this.loading = false
         })
      },
      handleSubmitForm() {
          this.$refs['dataForm'].validate(valid => {
              if (valid) {
                  this.loading = true
                  updateTrainEventSignInRecordInfo(this.editId, this.formData).then(response => {
