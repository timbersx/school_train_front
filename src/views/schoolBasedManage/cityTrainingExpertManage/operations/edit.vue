...
import { readCityExpertInfo, updateCityExpertInfo } from "@/api/schoolBasedManage/cityTrainingExpertManage";
export default {
  components: {
    Sticky,
    Tinymce
  ...
  methods: {
    init(){
        readCityExpertInfo(this.editId).then(res => {
    ...
    handleSave() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          updateCityExpertInfo(this.editId, this.trainingExpertData).then((res) => {
            this.$router.go(-1);
          });
        }
      });
